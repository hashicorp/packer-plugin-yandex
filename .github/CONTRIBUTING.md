...
runtime in order to build packer with the Yandex plugin.
...
## Setting up Yandex plugin for dev

With Go installed, you can already `go get` the Yandex plugin and `make dev` in
...
1. Download the Yandex plugin source (and its dependencies) by running
   `go get github.com/hashicorp/packer-plugin-yandex`. This will download the source to
   `$GOPATH/src/github.com/hashicorp/packer-plugin-yandex`.

2. When working on the Yandex plugin, first `cd $GOPATH/src/github.com/hashicorp/packer-plugin-yandex`
   so you can run `make dev` and easily access other files. `make dev` will build the packer-plugin-yandex binary and install it under `$HOME/.packer.d/plugins/`.

3. Make your changes to the Yandex plugin source. You can run `make dev` to build and install locally, and `make test` to run unit tests.
   Any compilation errors will be shown when the binaries are rebuilding. If you don't have `make` you can simply run `go build -o packer-plugin-yandex` from the project root, and `mv packer-plugin-yandex ~/.packer.d/plugins/packer-plugin-yandex` to install the plugin.

4. After building the Yandex plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/hashicorp/packer-plugin-yandex/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the Yandex plugin](https://github.com/hashicorp/packer-plugin-yandex#fork-destination-box), push your
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-yandex f-my-feature
```
...
3. One of Yandex plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Yandex plugin
project. After following the steps in "Setting up Go to work on the Yandex plugin":

1. Navigate to the code:

   `cd $GOPATH/src/github.com/hashicorp/packer-plugin-yandex`
...
2. Add the remote by running:
   ...
   `git remote add myuser https://github.com/myuser/packer-plugin-yandex.git`
...
Please only apply the minimal vendor changes to get your PR to work. The Yandex plugin
...
If the Yandex Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
